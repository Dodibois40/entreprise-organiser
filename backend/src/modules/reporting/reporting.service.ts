...
export class ReportingService {
  ...
  async getAffairesPerformance(params: {
    ...
    return affaires.map(affaire => {
      const totalAchats = affaire.bdc.reduce((sum, bdc) => sum + bdc.montantHt, 0);
      const totalFraisGeneraux = affaire.bdc.reduce((sum, bdc) => sum + bdc.montantFg, 0);
      const totalHeures = affaire.pointages.reduce((sum, pointage) => sum + pointage.nbHeures, 0);
  ...
  async getHeuresStats(params: {
    ...
    return {
      ...
      heuresParAffaire,
      heuresParUtilisateur,
      evolutionHeures: evolutionHeures.sort((a, b) => 
  ...
  async getAchatsEvolution(params: {
    ...
    return {
      ...
      achatsParCategorie,
      achatsParFournisseur,
      tendanceAchats: tendanceAchats.sort((a, b) => 
  ...
  async getMargesAffaires(params: {
    ...
    const margesCalculees = affaires.map(affaire => {
      const totalAchats = affaire.bdc.reduce((sum, bdc) => sum + bdc.montantHt, 0);
      const totalFg = affaire.bdc.reduce((sum, bdc) => sum + bdc.montantFg, 0);
      const totalHeures = affaire.pointages.reduce((sum, pointage) => sum + pointage.nbHeures, 0);
    ...
    return {
      ...
      resumeMarges: {
        margeMinimale: Math.min(...margesCalculees.map(m => m.tauxMarge)),
        margeMaximale: Math.max(...margesCalculees.map(m => m.tauxMarge)),
        margeMoyenne: margesCalculees.reduce((sum, m) => sum + m.tauxMarge, 0) / margesCalculees.length,
        nombreAffairesPositives: margesCalculees.filter(m => m.tauxMarge > 0).length,
        nombreAffairesNegatives: margesCalculees.filter(m => m.tauxMarge < 0).length,
      }
    };
  }
