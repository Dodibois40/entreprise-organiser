...
const bdcServiceFirebase = {
  ...
  async uploadPdfFirebase(bdcId, file, onProgress = null) {
    try {
      ...
      const firebaseResult = await firebaseStorageService.uploadBdcPdf(bdcId, file, onProgress);
      
      // 2. Mettre à jour le BDC dans la base de données avec les infos Firebase
      const updateData = {
