...
// Validation des données d'affaire
export const validateAffaire = (affaire) => {
  ...
  if (!affaire.objectifCaHt || affaire.objectifCaHt <= 0) {
    errors.objectifCaHt = 'L\'objectif CA HT est requis et doit être positif';
  }

  if (affaire.objectifAchatHt && affaire.objectifAchatHt < 0) {
    errors.objectifAchatHt = 'L\'objectif achat HT doit être positif ou nul';
  }

  // Validation des dates
  if (affaire.dateCommencement && affaire.dateCloturePrevue) {
    const dateCommencement = new Date(affaire.dateCommencement);
    const dateCloture = new Date(affaire.dateCloturePrevue);
    
    if (dateCloture < dateCommencement) {
      errors.dateCloturePrevue = 'La date de clôture ne peut pas être antérieure à la date de commencement';
    }
  }
